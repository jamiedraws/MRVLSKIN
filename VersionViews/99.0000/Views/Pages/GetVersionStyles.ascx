...
<%
    var cacheBusterVersion = 0;
    var v = string.Format("?v={0}", cacheBusterVersion);
%>

<link rel="preload" href="/images/main-offer.png" as="image" type="image/png">
...
<link rel="preload" href="/css/99.0000/shell.css<%= v %>" as="style">
<link rel="stylesheet" href="/css/99.0000/shell.css<%= v %>">

<% if (DtmContext.Page.IsStartPageType) { %>
    <link rel="preload" href="/css/99.0000/index.css<%= v %>" as="style">
    <link rel="stylesheet" href="/css/99.0000/index.css<%= v %>">
<% } %>

<% if (!DtmContext.Page.IsStartPageType) { %>
    <link rel="preload" href="/css/99.0000/dtm.css<%= v %>" as="style"/>
    <link rel="stylesheet" href="/css/99.0000/dtm.css<%= v %>" />
<% } %>

<script defer src="/js/99.0000/app.es5.js<%= v %>"></script>
